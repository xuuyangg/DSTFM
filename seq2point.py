...
# load the data set
# 假设 load_dataset 和 data_provider, model 已经正确导入
...
best_val_epoch = -1
writer = SummaryWriter(f'runs/{args.appliance_name}_training_logs')
best_val_loss = float('inf')
best_val_epoch = 0
...
mae = metric.get_abs_error(gt.flatten(), pred.flatten())
sae = metric.get_sae(gt.flatten(), pred.flatten(), sample_second)
sae_delta_1200 = metric.get_sae_delta(gt.flatten(), pred.flatten(), 1200)
sae_delta_600 = metric.get_sae_delta(gt.flatten(), pred.flatten(), 600)
test_metrics_info = f"MAE: {mae}\nSAE: {sae}\nSAE_Delta (1200): {sae_delta_1200}\nSAE_Delta (600): {sae_delta_600}"
test_writer = SummaryWriter(f'runs/{args.appliance_name}_test_metrics')
test_writer.add_text('Test Metrics', test_metrics_info, 0)
test_writer.close()
